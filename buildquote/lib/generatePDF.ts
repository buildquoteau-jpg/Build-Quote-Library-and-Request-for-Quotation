...
export async function generatePDFBuffer(payload: RFQPayload): Promise<Buffer> {
  ...
  function drawAllFooters() {
    ...
    pages.forEach((p, idx) => {
      ...
      const footerLine = `RFQ Reference: ${rfqId}   |   Sent: ${formatDate()}   |   buildquote.com.au   |   Page ${idx + 1} of ${totalPages}`
  ...
  function drawHeader(page: any): number {
    ...
    const deliveryLine = delivery === 'delivery'
      ...
      : 'Store Pick-up'
    const dateLine = `Date Required: ${formatDateRequired(dateRequired)}${projectRef}`
